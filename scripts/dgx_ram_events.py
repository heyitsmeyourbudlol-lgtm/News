...
def run_forever(*, log_fn: Callable[[str], None] = print) -> None:
    ...
    try:
        ...
        if isinstance(eff, dict):
            ...
            if isinstance(pri, dict):
                ...
                # 0.25–0.5GB crumbs — 1GB ballast + teacher preload OOM-killed
                # ram_events mid-climb (needle 2026-09-08: 23.8G peak → SIGKILL).
                pri["fill_chunk_gb"] = min(max(float(pri.get("fill_chunk_gb") or 0.25), 0.25), 0.5)
    ...
    while True:
        try:
            ...
            pri = auto.CFG.setdefault("ram_priority", {})
            if isinstance(pri, dict):
                # Keep crumbs small while GPU teacher is resident (unified GB10).
                pri["fill_chunk_gb"] = 0.25 if float(budget.mem_stats()["avail_gb"]) < 20 else 0.5
                pri["min_fill_avail_gb"] = min(
                    max(float(pri.get("min_fill_avail_gb") or 4.5), 4.5), 5.5
                )
