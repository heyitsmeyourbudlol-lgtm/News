...
# with _PROBE_TTL_SEC so write_team/dual remiss share one scan per window.
...
def scan_bottlenecks(*, daemons: dict[str, bool] | None = None) -> list[Bottleneck]:
    ...
    ``_PROBE_TTL_SEC``; explicit ``daemons=`` bypasses (tests / injected paths).
    """
    if daemons is None:
        now = time.monotonic()
        cached = _SCAN_BOTTLENECKS_CACHE.get("items")
        at = float(_SCAN_BOTTLENECKS_CACHE.get("at") or 0.0)
        if cached is not None and (now - at) < _PROBE_TTL_SEC:
            return list(cached)
